...
class SongService:
    ...
    @staticmethod
    def get_cluster_for_input(base_features: dict,
                          danceability_contrast: float = 0.0,
                          energy_contrast: float = 0.0,
                          loudness_contrast: float = 0.0) -> int:
        """
    ...
    def get_recommendations(base_song_id: int,
        ...
        # Determine cluster:
        if (danceability_contrast == 0.0 and energy_contrast == 0.0 and loudness_contrast == 0.0):
            # Use the stored cluster if available
            cluster = base_song.cluster
            if cluster is None:
                raise ValueError("Base song has no cluster assigned. Run clustering first.")
        else:
            base_features = {
            ...
            cluster = SongService.get_cluster_for_input(base_features,
                                            danceability_contrast,
                                            energy_contrast,
                                            loudness_contrast)
